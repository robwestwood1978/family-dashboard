...
test("enforces the read-only preview at every interactive control boundary", async ({ page }) => {
  ...
  await expect(card.locator(".preview-pill")).toHaveText(/Read-only preview/);
  ...
  await card.locator('.nav-button[data-view="family"]').click();
  await expect(card.locator(".map-panel")).toContainText("Location sharing is disabled for this preview");
