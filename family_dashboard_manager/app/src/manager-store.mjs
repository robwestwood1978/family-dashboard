...
export class DashboardStore {
  ...
  validate(candidate) {
    ...
    if (this.requireReadOnly && config.display.read_only !== true) {
      throw new Error("this v0.4 release requires config.display.read_only to be true");
    }
  ...
  getResourceUrl() {
    return `${this.publicResourceBase}/family-hub-card.js?v=${process.env.APP_VERSION || "0.4.2"}`;
  }
  ...
  async getStatus() {
    ...
    return {
      app_version: process.env.APP_VERSION || "0.4.2",
